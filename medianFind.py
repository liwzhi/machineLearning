...
class KeplerMapper(object):
  ...
  def visualize(self, complex, path_html="mapper_visualization_output.html", title="My Data", graph_link_distance=30, graph_gravity=0.1, graph_charge=-120, custom_tooltips=None):
    ...
    * {margin: 0; padding: 0;}
    ...
    #holder { position: relative; width: 1200px; height: 800px; background: #111; display: block;}
    ...
    <div id="holder">
      ...
      <!-- This creates spurious edges <b>Linking locally</b><br>%s<br><br> -->
...
if __name__ == "__main__":
  ...
  features = [c for c in train.columns if c not in ["id", "mass", "signal", "production", "min_ANNmuon", "SPDhits"]]
  data = np.array(train[features])
  y = np.array(train["signal"])

  mapper = KeplerMapper(cluster_algorithm=cluster.DBSCAN(eps=1, min_samples=5), 
                           nr_cubes=30, overlap_perc=0.1, 
  ...
  complex = mapper.map(data, dimension_index=[25,26,27], 
                       dimension_name="hypercube(%s * %s * %s)"%(features[25],features[26],features[27]))

  mapper.visualize(complex, "results.html", "CERN τ → μμμ", 
                   graph_gravity=0.25, graph_charge=-60, custom_tooltips=y)

...
colName = ['Weekday','DepartmentDescription']
...
aa = X.groupby('VisitNumber').mean()  
aa['Count'] = X.groupby('VisitNumber').count()['Weekday']
ab = X.groupby('VisitNumber').std()
y = aa['TripType']
del aa['TripType']
...
bb = X_test.groupby('VisitNumber').mean()
bb['Count'] = X_test.groupby('VisitNumber').count()['Weekday']
...
params = {"objective": "multi:softprob",
          ...
          "seed": 1}
num_trees=450
gbm = xgb.train(params, xgb.DMatrix(X_train, yy), num_trees)
#%% validatoin 
from sklearn import cross_validation
from sklearn.metrics import accuracy_score,roc_auc_score,confusion_matrix
